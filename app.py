...
from modules.reply import faq,menu#引用同一個資料夾的檔案
from modules.currency import get_exchange_table
import os
app = Flask(__name__)
channel_secret='c45dd87800158717ca9bd742867da6b1'
channel_token='LqoVBK0lslTlSc8TJTgICBuk3pbDuDXl2DcjVBhvOFG7HnpTNUbG5ddR4qROcVfvn3/6ywSkUCEc8tbJG+cy6pWve5ilD+gr7h2VfCs76CxpyWYLa7dQoEvvZOWqU25o4cZFU+2YgRSq1TtInt1OYgdB04t89/1O/w1cDnyilFU='
table=get_exchange_table()
configuration = Configuration(access_token=channel_token)#YOUR_CHANNEL_ACCESS_TOKEN
handler = WebhookHandler(channel_secret)#channel sercert
...
def handle_message(event):
    ...
    with ApiClient(configuration) as api_client:
        ...
        elif user_msg.lower() in ["選單"]:
            bot_msg=menu
        elif user_msg in table:
            buy=table[user_msg]["buy"]
            sell=table[user_msg]["sell"]
            bot_msg=TextMessage(text=f"{user_msg} buy:{buy} sell:{sell}")
